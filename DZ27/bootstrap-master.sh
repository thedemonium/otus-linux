...
DBDUMP_FILE=/vagrant/config/master/bet.dmp
...
then
	...
	echo -e "--- Import MySQL DUMP file ---"
	sudo mysql -uroot -p$DBPASSWD -D bet < $DBDUMP_FILE
...
then
	...
	# Get database dump
	echo -e "---- Dump database to SQL"
	mysqldump --all-databases --triggers --routines --master-data --ignore-table=bet.events_on_demand --ignore-table=bet.v_same_event -u$DBUSER -p$DBPASSWD > /vagrant/config/$DBNAME.sql
	...
	sshpass -p "$REPLICA_SSH_PASS" ssh -o StrictHostKeyChecking=no $REPLICA_SSH_USER@$REPLICA_IP "mysql -u$DBUSER -p$DBPASSWD $DBNAME < /vagrant/config/$DBNAME.sql"
	...
	sshpass -p "$REPLICA_SSH_PASS" ssh -o StrictHostKeyChecking=no $REPLICA_SSH_USER@$REPLICA_IP "mysql -u$DBUSER -p$DBPASSWD -e \"SLAVE STOP;CHANGE MASTER TO MASTER_HOST='$MASTER_IP', MASTER_USER='repl', MASTER_PASSWORD='mysqluser', MASTER_LOG_FILE='$CURRENT_LOG', MASTER_LOG_POS=$CURRENT_POS;START SLAVE;\""
